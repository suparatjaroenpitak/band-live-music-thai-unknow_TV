...
type FilterMode = "All" | "Guitar" | "Bass" | "Piano" | "Strings" | "Synth" | "Choir" | "Favorites" | "Recently Used";

const FILTERS: FilterMode[] = ["All", "Guitar", "Bass", "Piano", "Strings", "Synth", "Choir", "Favorites", "Recently Used"];

export const InstrumentBrowser = memo(function InstrumentBrowser() {
  ...
  const filtered = useMemo(() => {
    ...
    return INSTRUMENTS.filter((instrument) => {
      if (filter === "Favorites" && !favoriteInstruments.includes(instrument.id)) return false;
      if (filter === "Recently Used" && !recentInstruments.includes(instrument.id)) return false;
      if (!["All", "Favorites", "Recently Used"].includes(filter) && instrument.category !== filter) return false;
      return (
        instrument.name.toLowerCase().includes(needle) ||
        instrument.family.includes(needle) ||
        instrument.category.toLowerCase().includes(needle) ||
  ...
  const selectInstrument = useCallback(
    ...
    [bpm, mixer, setCurrentInstrument]
  );
    ...
    <section className="relative">
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block truncate text-xs text-slate-400">{selected.category}</span>
        </span>
        ...
        {open ? (
          ...
          >
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
              {FILTERS.map((item) => (
                <FilterButton key={item} active={filter === item} label={item} onClick={() => setFilter(item)} />
              ))}
            </div>

            <div
              className="scrollbar-thin mt-3 h-[316px] overflow-auto rounded-lg border border-white/10 bg-black/15"
                ...
                <div style={{ transform: `translateY(${list.offsetY}px)` }}>
                  {list.indexes.map((index) => {
                    ...
                    return (
                      <button
                        key={instrument.id}
                        className={`flex h-[60px] w-full items-center gap-3 px-3 text-left transition ${
...
function FilterButton({ active, label, onClick }: { active: boolean; label: FilterMode; onClick: () => void }) {
  const icon = label === "Favorites" ? <Heart size={15} /> : label === "Recently Used" ? <Clock3 size={15} /> : <Guitar size={15} />;

  return (
    <button
      className={`flex h-9 shrink-0 items-center justify-center gap-2 rounded-lg border px-3 text-xs font-semibold transition ${
